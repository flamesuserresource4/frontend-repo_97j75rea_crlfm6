...
export default function SoilClassifier() {
  ...
  const submitForClassification = async () => {
    ...
    try {
      ...
      form.append('image', blob, 'soil.jpg');
      form.append('file', blob, 'soil.jpg'); // compatibility with backends expecting `file`

      const resp = await fetch(`${API_BASE}/predict_soil`, {
        method: 'POST',
        body: form,
      });
  ...
  return (
    <section className="bg-white/80 backdrop-blur border border-gray-200 rounded-2xl p-6 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-semibold">Soil Classification (Camera)</h2>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-3">
          <div className="aspect-video w-full rounded-xl overflow-hidden border border-gray-200 bg-black/5 grid place-items-center relative">
            {!streaming && !previewUrl && (
              <p className="text-gray-500 text-sm">Point the camera close to the soil surface with good lighting</p>
            )}
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button onClick={submitForClassification} disabled={loading} className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-emerald-700 text-white disabled:opacity-60">
        ...
        <div className="bg-emerald-50/60 border border-emerald-200 rounded-xl p-4">
          ...
          {!result && <p className="text-sm text-emerald-900/80">Capture or upload soil image and press Classify. The server will identify soil type and suggestions.</p>}
          {result && (
            <div className="space-y-2">
              ...
              {result.properties && (
                <pre className="text-xs bg-white/70 border border-emerald-200 rounded-lg p-3 overflow-auto"><code>{JSON.stringify(result.properties, null, 2)}</code></pre>
              )}
